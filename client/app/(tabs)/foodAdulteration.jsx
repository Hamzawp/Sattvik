...
export default function FoodAdulteration() {
  const [image, setImage] = useState(null);
  const [loading, setLoading] = useState(false);
  const [similarity, setSimilarity] = useState(null); // State to hold similarity result
  ...
  const saveProductDetail = async (imageUrl) => {
    await setDoc(doc(db, "FoodAdulterationImages", Date.now().toString()), {
      imageUrl: imageUrl,
    });
  ...
  const fetchSimilarity = async (imageUrl) => {
    try {
      const response = await fetch("http://10.0.2.2:5000/process-openimage", {
        method: "POST",
        headers: {
  ...
  return (
    <ScrollView style={styles.scrollContainer}>
      ...
      {similarity && ( // Display similarity result if available
